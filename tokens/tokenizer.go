...
func BaseConfig() TokenizerConfig {
	cfg := TokenizerConfig{
		...
		Keywords: map[string]TokenType{
			...
			"JSON":              JSON,
			"JSONB":             JSONB,
			// TODO(slice5): HSTORE belongs in the Postgres tokenizer only (postgres.py:92),
			// not base (upstream base tokens.py has no HSTORE). Move it to the Postgres
			// KEYWORDS override when the Postgres dialect is wired; base/MySQL must not
			// recognize HSTORE as a type keyword.
			"HSTORE":         HSTORE,
			"KEEP":           KEEP,
			"KILL":           KILL,
			"LATERAL":        LATERAL,
